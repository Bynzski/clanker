...
describe('selectors', () => {
  describe('usePat', () => {
    it('returns null when no PAT stored', () => {
      const pat = useVcsStore.getState().storedPats.github;
      expect(pat).toBeNull();
    });
    ...
    it('returns stored PAT', () => {
      ...
      expect(useVcsStore.getState().storedPats.github).toEqual(pat);
    });
  });

  describe('useSshKey', () => {
    it('returns initial SSH key status', () => {
      const sshKey = useVcsStore.getState().sshKey;
      expect(sshKey.exists).toBe(false);
    });

    it('returns updated SSH key status', () => {
      getStore().setSshKey({ exists: true, fingerprint: 'SHA256:abc' });
      expect(useVcsStore.getState().sshKey.exists).toBe(true);
    });
  });

  describe('useCredentialsLoading', () => {
    it('returns initial loading state', () => {
      expect(useVcsStore.getState().isLoading).toBe(false);
    });

    it('returns updated loading state', () => {
      getStore().setLoading(true);
      expect(useVcsStore.getState().isLoading).toBe(true);
    });
  });

  describe('useCredentialsError', () => {
    it('returns initial error state', () => {
      expect(useVcsStore.getState().error).toBeNull();
    });

    it('returns updated error state', () => {
      getStore().setError('Test error');
      expect(useVcsStore.getState().error).toBe('Test error');
    });
  });

  describe('useProviderContext', () => {
    it('returns initial context', () => {
      const ctx = useVcsStore.getState();
      expect(ctx.provider).toBeNull();
      expect(ctx.pullRequest).toBeNull();
      expect(ctx.deepLinks).toEqual([]);
      expect(ctx.isLoading).toBe(false);
      expect(ctx.error).toBeNull();
    });

    it('returns populated context', () => {
      getStore().setProviderContext({
        provider: 'github',
        baseUrl: 'https://github.com',
        owner: 'owner',
      ...
      const ctx = useVcsStore.getState();
      expect(ctx.provider?.provider).toBe('github');
      expect(ctx.pullRequest?.number).toBe(42);
      expect(ctx.deepLinks).toHaveLength(1);
